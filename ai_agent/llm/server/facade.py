...
from ai_agent.llm.server.protocol import info_payload
...
class AgentLlmFacade:
    """Stable HTTP surface for agent clients; delegates to a server-side engine."""
    ...
    def info(self) -> dict[str, str]:
        return info_payload(
            ...
            upstream=self.engine.upstream_url,
        )
    ...
    def chat_lines(self, payload: dict[str, Any]) -> Iterator[str]:
        yield from self.engine.chat_lines(payload)


def bind_llm_server(
    host: str,
    port: int,
    facade: AgentLlmFacade,
) -> ThreadingHTTPServer:
    return ThreadingHTTPServer((host, port), _make_handler(facade))


def _make_handler(facade: AgentLlmFacade) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if path in {"/", "/health"}:
                self._send_json(
                    ...
                    {
                        "status": "ok",
                        "engine": facade.engine.engine_name,
                        "model": facade.engine.model,
                        "upstream": facade.engine.upstream_url,
                    },
                )
                return
            if path == "/api/info":
                self._send_json(200, facade.info())
                return
            if path == "/api/tags":
                try:
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                logger.warning("Engine chat failed after stream started: %s", exc)
